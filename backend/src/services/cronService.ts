...
import { sendDataMessage } from './fcmService';
...
async function sendAutoReminder(): Promise<void> {
  try {
    ...
    // Try socket first, fall back to FCM
    ...
    console.log(`[cron] Auto-reminder sent (socket: ${delivered}, fcm: ${!delivered})`);
  } catch (err) {
...
export function startCronScheduler(): void {
  ...
  cron.schedule('0 7 * * *',   sendAutoReminder, { timezone: 'Asia/Kolkata' });
  cron.schedule('30 8 * * *',  sendAutoReminder, { timezone: 'Asia/Kolkata' });
  cron.schedule('0 10 * * *',  sendAutoReminder, { timezone: 'Asia/Kolkata' });
  cron.schedule('30 11 * * *', sendAutoReminder, { timezone: 'Asia/Kolkata' });

  // Peak Metabolism Phase: 12:00 PM - 5:00 PM IST, every 90 minutes
  // Fires at: 13:00, 14:30, 16:00
  cron.schedule('0 13 * * *',  sendAutoReminder, { timezone: 'Asia/Kolkata' });
  cron.schedule('30 14 * * *', sendAutoReminder, { timezone: 'Asia/Kolkata' });
  cron.schedule('0 16 * * *',  sendAutoReminder, { timezone: 'Asia/Kolkata' });

  // Wind-down Phase: 5:00 PM - 10:00 PM IST, every 120 minutes
  // Fires at: 17:30, 19:30, 21:30
  cron.schedule('30 17 * * *', sendAutoReminder, { timezone: 'Asia/Kolkata' });
  cron.schedule('30 19 * * *', sendAutoReminder, { timezone: 'Asia/Kolkata' });
  cron.schedule('30 21 * * *', sendAutoReminder, { timezone: 'Asia/Kolkata' });
